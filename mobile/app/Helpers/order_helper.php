...
function cloud_confirmorder($order_id)
{
	if (0 < $order_id) {
		...
		if ($cloud_order) {
			...
			$cloud_orders['notifyUrl'] = $GLOBALS['ecs']->get_domain() . 'api.php?app_key=' . C('shop.cloud_dsc_appkey') . '&method=dsc.order.confirmorder.post&format=json&interface_type=1';
			$cloud = new \App\Services\Erp\JigonService();
