...
function env_check(&$env_items) {
	...
	$env_items[] = array('name' => 'PHP版本', 'min' => '5.6', 'good' => '5.6', 'cur' => PHP_VERSION, 'status'=>(PHP_VERSION < 5.3 ? 0:1));
	$tmp = function_exists('gd_info') ? gd_info() : array();
	preg_match("/[\d.]+/", $tmp['GD Version'],$match);
	unset($tmp);
	$env_items[] = array('name' => 'GD库', 'min' => '2.0', 'good' => '2.0', 'cur' => $match[0], 'status' => ($match[0] < 2 ? 0:1));
	$env_items[] = array('name' => '附件上传', 'min' => '未限制', 'good' => '20M','cur' => ini_get('upload_max_filesize'), 'status' => 1);
	$disk_place = function_exists('disk_free_space') ? floor(disk_free_space(ROOT_PATH) / (1024*1024)) : 0;
	$env_items[] = array('name' => '磁盘空间', 'min' => '500M', 'good' => '>500M','cur' => empty($disk_place) ? '未知' : $disk_place.'M', 'status' => ($disk_place < 100 and $disk_place > 5) ? 0:1);
}
/**
 * file check
 */
