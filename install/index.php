...
<div class="header">
  <div class="layout">
    ...
    <div class="version">版本: 2018.05.04.0002</div>
  </div>
</div>
EOF;
...
$html_footer = <<<EOF
<div class="footer">
  <h5>Powered by <font class="blue">DM299</font><font class="orange"></font></h5>
  <h6>版权所有 2016-2018 &copy; <a href="http://dm299.taobao.com" target="_blank">商创网络</a></h6>
</div>
EOF;
...
function step3(&$install_error,&$install_recover){
    ...
    $sq = trim($_POST['sq']);
    if (!$db_host || !$db_port || !$db_user || !$db_pwd || !$db_name || !$sq ){
        $install_error = '输入不完整，请检查';
    }
    if ( !preg_match("/^[0-9]{15,18}$/", $sq)  ){
        $install_error = '授权号:输入不完整，请检查';
    }
...
*/
	...
	if( file_exists('./data/utf8_add.sql') ) {
	...
	*/
    ...
    $fp = @fopen('lock','wb+');
    @fclose($fp);
    exit("<script type=\"text/javascript\">document.getElementById('install_process').innerHTML = '安装完成，下一步...';document.getElementById('install_process').href='index.php?step=4&sitename={$sitename}&username={$username}&password={$password}';</script>");
    exit();
}
//execute sql
...
function write_config($url) {
    ...
    $configfile = str_replace("===dm299_version===", $dm299_version, $configfile);
    @file_put_contents('../data/config.php', $configfile);
    @file_put_contents('../mobile/supplier/data/config.php', $configfile);
}